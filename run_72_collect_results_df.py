...
ds_dir = os.path.join(ds_root_path, 'learning_out_test/pdfs')

os.chdir(ds_dir)
df = None
for reg in ['01_split_results_error', '03_split_regFDresults_error']:
...
for reg_path, reg in {'01_split_predicted': False, '03_split_regFDpredicted': True}.items():
    scatter_df[reg] = {}
    for a in ana_list:
        scatter_df[reg][a] = pd.read_pickle(os.path.join(ds_dir, reg_path, a + '_df_predicted.pkl'))

all_ana = set(df.query('MAE_test < 20').index)
...
good_ana = set(df.query('MAE_test < 6').index)
...
c = sns.color_palette()
plt.figure()
ax = sns.barplot(x="MAE_test", y="ana", data=df_good, hue='FD_res')
#plt.tight_layout()
plt.savefig('00_best7_bars_MAE.pdf')

plt.figure()
ax = sns.barplot(x="r2_test", y="ana", data=df_good, hue='FD_res')
#plt.tight_layout()
plt.savefig('00_best7_bars_r2.pdf')
...
plt.tight_layout()
plt.savefig('00_best7_scatters.pdf')
