...
for subject_id in subjects_list:
    print(subject_id)
    for bp in bp_list:
        m_file = '/scr/adenauer2/Franz/LeiCA_LIFE/metrics/{subject_id}/con_mat/matrix/{bp}/gordon/matrix.pkl'.format(
            subject_id=subject_id, bp=bp)
        m = pickle.load(open(m_file))['correlation']

        out_path = '/scr/adenauer2/Franz/LeiCA_LIFE/metrics/{subject_id}/con_mat/matrix/{bp}/gordon/'.format(
            subject_id=subject_id, bp=bp)
        ...
        # # shifted matrix (values betw 0 and 1)
        # suf_str = '_shifted'
        # m_ds = dict()
        # m_shift = (m + 1) / 2.
        # m_ds['correlation'] = ds_mat(m_shift, mapping)
        # out_file = os.path.join(out_path, 'matrix_downsampled%s.pkl' % suf_str)
        # pickle.dump(m_ds, open(out_file, 'w'))
