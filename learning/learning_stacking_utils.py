...
def stacking(root_path, out_path, target, selection_crit, source_dict, source_selection_dict, rf=None):
    ...
    for stacking_crit in source_selection_dict.keys():
        ...
        for a in ['select', 'y_predicted_cv']:
            if a not in df_all:
                df_all[a] = np.nan
        ...
        scores_train = pd.DataFrame({'cv_r2': [cv_results['cv_r2']],
                                     'cv_mae': [cv_results['cv_mae']]}, index=[stacking_crit])
